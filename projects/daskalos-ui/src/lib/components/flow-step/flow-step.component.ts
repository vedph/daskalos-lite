...
export class FlowStepComponent implements OnDestroy {
  private _step: FlowStep | undefined;
  ...
  public timerMessage: string | undefined;
   ...
   */
  @Input() public get step(): FlowStep | undefined {
    return this._step;
  }
  public set step(value: FlowStep | undefined) {
    this.stopTimer(false);

    this._step = value;
    this.initChoiceControls();

    if (value && (value.timeAllotted || 0) > 0) {
      this.timeRunning = true;
      this._secondsToGo = value.timeAllotted || 0;
      this.startTimer();
    } else {
  ...
  constructor(
    ...
    this._timerTicks$ = this._timerIntervals$.pipe(
      switchMap((interval) =>
        interval ? timer(0, interval).pipe(scan((a, v, i) => a + 1, 0)) : EMPTY
      )
    );
  ...
  private addChoiceControl(choice: FlowChoice) {
    const group = this._formBuilder.group({
      checked: this._formBuilder.control(choice?.checked),
      content: this._formBuilder.control(choice?.freeContent),
    });
  ...
  private initChoiceControls() {
    ...
    if (this._step.maxChoices === 1) {
      this.stepGroup.addControl('radioChoice', this._formBuilder.control(null));
    } else {
  ...
  public onStepForward(force = false) {
    ...
    let checkedCount = 0;
    let singleChoice: number = -1;

    if (this._step.maxChoices === 1 && this._step.choices?.length) {
      singleChoice = this.stepGroup.controls['radioChoice'].value;
    }
    ...
    const answer: FlowUserStep = {
      number: this.step?.number as number,
      label: this.step?.label as string,
      prompt: this.step?.prompt as string,
      choices: userChoices,
    };
