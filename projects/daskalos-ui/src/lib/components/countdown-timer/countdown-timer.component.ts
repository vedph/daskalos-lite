...
export class CountdownTimerComponent implements OnDestroy {
  ...
  private _targetDate: string | undefined;
  private _secondsToGo: number | undefined;
   ...
   * prefixed by + (e.g. "+15").
   */
  @Input() get targetDate(): string | undefined {
    return this._targetDate;
  }
  set targetDate(value: string | undefined) {
  ...
  @Input() get running(): boolean {
    return this._running? true : false;
  }
  ...
  constructor() {
    ...
    this._tickSubcription = this._timerTicks$.subscribe(n => {
      ...
      const left = (this._secondsToGo || 0) - n;
      if (left <= 0) {
        this.stop(true);
      }
      // update the display
